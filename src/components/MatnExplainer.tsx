...
export default function MatnExplainer({
  ...
  return (
    <div className="space-y-6">
      {/* Book selector */}
      <div className="bg-bg-secondary rounded-3xl p-6 shadow-sm border border-brand-primary/10 space-y-4">
        <div>
          <h3 className="text-sm font-black text-brand-primary mb-2.5 flex items-center gap-1.5 selection:bg-amber-100">
            ...
            {Object.keys(EXCLUSIVE_MATNS).map((name) => (
              <button
                ...
                className={`text-xs font-black p-3.5 rounded-2xl transition-all duration-200 text-center border cursor-pointer ${
                  matnName === name
                    ? "bg-brand-primary text-white scale-102 border-brand-primary shadow-sm"
                    : "bg-brand-light/40 text-brand-primary border-brand-primary/10 hover:bg-brand-primary hover:text-white"
                }`}
              >
                {name}
              </button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-brand-primary text-white rounded-3xl p-6 shadow-md relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-3">
              ...
              <h2 className="text-xl md:text-2xl font-black font-amiri tracking-wide text-right">
                {selectedData.name}
              </h2>
              ...
              <p className="text-xs text-emerald-100 font-normal leading-relaxed leading-5 border-t border-white/10 pt-2.5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-bg-secondary rounded-3xl p-5 shadow-sm border border-brand-primary/5 space-y-3 text-right">
              <h4 className="text-xs font-black text-brand-primary border-b border-border-primary pb-2 flex items-center gap-1.5">
                <Compass className="w-4 h-4 text-amber-500" />
                أبواب وفصول المنظومة المعتمدة:
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-bold text-text-secondary">
                {selectedData.chapters.map((chapter) => (
                  <span
                    key={chapter}
                    className="p-2 bg-bg-tertiary border border-brand-primary/5 rounded-xl block truncate pr-2.5 text-right border-r-2 border-r-amber-500/40"
                  >
                    {chapter}
                  </span>
                ))}
            ...
            <div className="bg-bg-secondary rounded-3xl p-5 shadow-sm border border-brand-primary/5 space-y-3 text-right">
              ...
              <ul className="text-xs font-bold text-text-secondary space-y-2 pr-1">
                {selectedData.benefits.map((benefit, idx) => (
                  <li
                    key={idx}
                    className="text-right leading-relaxed pr-4.5 relative before:content-['•'] before:absolute before:right-0 before:text-sm before:text-amber-500"
                  >
                    {benefit}
                  </li>
                ))}
          ...
          <div className="bg-bg-secondary rounded-3xl p-6 shadow-sm border border-brand-primary/10 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border-primary pb-4">
              <div className="flex items-center gap-2">
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-text-muted font-semibold">
                    تصفح الأبيات بجميع حركاتها وتشكيلها الصحيح، وانقر على أي بيت
                    لشرحه وإعرابه بالكامل
                  </p>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full sm:w-48 pl-3 pr-9 py-1.5 text-xs bg-bg-tertiary border border-border-primary rounded-xl outline-none focus:border-brand-primary text-right font-bold text-text-secondary"
                />
              </div>
            </div>

            {error && (
              <p className="text-red-500 text-xs font-bold text-center py-1">
                {error}
              </p>
            )}
            ...
            <div
              ...
              id="interactive-verses-canvas"
            >
              {filteredVerses.map((verse) => {
                const isSelected = explainingId === verse.id;
                const realIndex =
                  selectedData.verses.findIndex((v) => v.id === verse.id) + 1;
                ...
                return (
                  <div
                    ...
                    className={`p-5 rounded-2xl border transition-all duration-300 space-y-4 ${
                      isSelected
                        ? "bg-brand-light/40 border-amber-400 shadow-md scale-[1.01]"
                        : "bg-bg-tertiary border-border-primary/50 hover:border-emerald-600/30 hover:bg-bg-tertiary"
                    }`}
                  >
                    <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
                      {/* Left-side / top verse numbering badge */}
                      <div className="flex items-center gap-2.5 self-start lg:self-center">
                        <div className="w-8 h-8 rounded-full bg-brand-primary text-white text-xs font-black flex items-center justify-center shadow-xs">
                          {realIndex}
                        </div>
                        <span className="text-[10px] bg-bg-tertiary text-text-secondary px-2 py-0.5 rounded-full font-bold">
                          {verse.chapter}
                        </span>
                      </div>
                      ...
                      <div className="flex-1 w-full flex flex-col md:flex-row items-stretch justify-between gap-3 text-right">
                        {/* Shatr 1 */}
                        <div className="flex-1 font-amiri text-base md:text-lg lg:text-xl text-white bg-brand-primary px-4 py-3 rounded-2xl shadow-2xs font-extrabold flex items-center justify-center text-center leading-loose">
                          {verse.s1}
                        </div>
                      ...
                      <div className="flex gap-2 shrink-0 self-end lg:self-center">
                        <button
                          onClick={() =>
                            handleSpeak(`${verse.s1} ، ${verse.s2}`)
                          }
                          className="p-2 bg-bg-secondary border border-border-primary hover:border-gold-accent hover:bg-amber-50 rounded-xl hover:scale-105 transition-all text-brand-primary cursor-pointer shadow-2xs"
                    ...
                    {isSelected && (
                      <div className="border-t border-border-primary/80 pt-4 space-y-4">
                        {explainLoading && (
                          <ProgressTimer
                            message="جاري طلب الشرح اللغوي التفصيلي وإذن الإعراب النحوي من المعلم..."
                            estimatedSeconds={10}
                          />
                        )}

                        {explanation && !explainLoading && (
                          <div className="space-y-4 text-xs font-medium leading-relaxed text-text-secondary text-right">
                            {/* Two-column responsive layout for major explanations */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {/* 1. Vocabulary explanation */}
                              <div className="bg-bg-tertiary p-4.5 rounded-2xl border border-amber-200/40 space-y-2 shadow-2xs">
                                ...
                                <p className="text-text-secondary leading-relaxed text-[11px] font-bold whitespace-pre-line leading-loose">
                              ...
                              {/* 2. Overall meaning */}
                              <div className="bg-bg-tertiary p-4.5 rounded-2xl border border-brand-primary/10 space-y-2 shadow-2xs">
                                <h5 className="font-black text-brand-primary text-xs flex items-center gap-1.5 border-b border-emerald-100 pb-1.5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              <div className="bg-bg-secondary p-4.5 rounded-2xl border border-border-primary/60 space-y-2 shadow-2xs">
                                <h5 className="font-black text-brand-primary text-xs flex items-center gap-1.5 border-b border-border-primary pb-1.5">
                                  <span>📚</span> الفوائد والتعليمات المستخرجة:
                                </h5>
                                <ul className="space-y-1.5 pr-2.5">
                                  {(
                                    explanation.scientificBenefits || [
                                      explanation.meaning,
                                    ]
                                  ).map((benefit, idx) => (
                                    <li
                                      key={idx}
                                      className="text-text-secondary text-[11px] font-bold pr-4 relative before:content-['•'] before:absolute before:right-0 before:text-brand-primary before:font-bold leading-relaxed"
                                    >
                                      {benefit}
                                    </li>
                                  ))}
                              ...
                              <div className="bg-bg-secondary p-4.5 rounded-2xl border border-border-primary/60 space-y-2 shadow-2xs">
                                ...
                                <ul className="space-y-1.5 pr-2.5">
                                  {(explanation.practicalExamples || [])
                                    .length > 0 ? (
                                    explanation.practicalExamples?.map(
                                      (example, idx) => (
                                        <li
                                          key={idx}
                                          className="text-text-secondary text-[11px] font-bold pr-4 relative before:content-['✓'] before:absolute before:right-0 before:text-brand-primary before:font-black leading-relaxed"
                                        >
                                          {example}
                                        </li>
                                      ),
                                    ...
                                    <li className="text-text-muted text-[11px] italic pr-4 relative before:content-['✓'] before:absolute before:right-0 before:text-brand-primary leading-relaxed">
                                      {explanation.meaning}
                                    </li>
                                  )}
                                </ul>
                            ...
                            {/* Grammar Analysis */}
                            <div className="bg-brand-light/20 p-4.5 rounded-2xl border border-brand-primary/10 space-y-2 shadow-2xs">
                              <h5 className="font-black text-brand-primary text-xs border-b border-emerald-100 pb-1.5">
                                💬 الإعراب البلاغي والنحوي للشطرين ميسرًا:
                              </h5>
                              <p className="font-mono text-text-secondary text-[11px] leading-loose font-semibold whitespace-pre-line leading-loose">
                            ...
                            {explanation.tajweedRules?.length > 0 && (
                              <div className="bg-bg-tertiary p-4.5 rounded-2xl border border-brand-primary/5 space-y-2 shadow-2xs">
                                <h5 className="font-black text-amber-600 text-xs border-b border-border-primary pb-1.5">
                                  ✨ الأحكام التجويدية والفوائد المصاحبة:
                                </h5>
                                <ul className="list-disc list-inside space-y-1.5 pr-1 text-[11px]">
                                  {explanation.tajweedRules.map((rule, idx) => (
                                    <li
                                      key={idx}
                                      className="list-none pr-4 relative before:content-['✓'] before:absolute before:right-0 before:font-bold before:text-brand-primary leading-relaxed"
                                    >
                                      {rule}
                                    </li>
                                  ))}
                            ...
                            <div className="p-4 bg-amber-500/10 text-amber-900 rounded-2xl border border-gold-accent/15 shadow-2xs">
                              <h5 className="font-black text-gold-accent text-xs">
                                💡 نصيحة دراسية للحفظ وضبط اللحن:
                              </h5>
                              <p className="font-serif italic text-[11px] mt-1 pr-1.5 leading-relaxed font-bold">
                                {explanation.educationalTip}
                              </p>
                            </div>
                          </div>
                        )}
              ...
              {filteredVerses.length === 0 && (
                <p className="text-center text-xs text-text-muted py-6">
                  مكرم! لم نعثر على أي شطر شعر موازٍ لبحثك.
                </p>
              )}
            </div>
          </div>
