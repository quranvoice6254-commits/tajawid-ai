...
export default function SmartAssistant({
    ...
    try {
      ...
      if (isFirebaseReady && db && !user?.isAnonymous) {
        ...
        fetched = fetched.filter(
          (s: any) => s.userId === (user?.uid || "local-user"),
        );
      }
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      if (isFirebaseReady && db && !user?.isAnonymous) {
        const { addDoc, collection, doc, updateDoc } =
          await import("firebase/firestore");
  ...
  return (
    <div
      className="grid grid-cols-1 md:grid-cols-4 bg-bg-secondary rounded-3xl overflow-hidden border border-brand-primary/15 shadow-sm min-h-[550px]"
      id="smart-chat-assistant-panel"
    >
      {/* Session histories sidebar */}
      <div className="md:col-span-1 bg-bg-tertiary p-4 border-l border-border-primary flex flex-col justify-between space-y-4">
        <div className="space-y-3 flex-1 flex flex-col overflow-hidden max-h-[480px]">
          <div className="flex items-center justify-between">
            <button
              onClick={() => setShowArchive(!showArchive)}
              className="text-[10px] font-black text-brand-primary px-2 py-1 bg-brand-light rounded-lg hover:bg-brand-primary/20 transition-colors cursor-pointer"
            >
              الأرشيف {showArchive ? "▼" : "◀"}
            </button>
            <button
              ...
              className="p-1 text-brand-primary hover:bg-brand-light rounded-lg transition-colors"
          ...
          {showArchive && (
            <div className="space-y-1.5 overflow-y-auto custom-scrollbar flex-1 pr-1 pl-1">
              {sessions.map((s) => (
                ...
                >
                  <div className="flex-1 overflow-hidden">
                    ...
                    <span
                      className={`text-[8px] font-medium block mt-1 ${currentSessionId === s.id ? "text-emerald-100" : "text-text-muted"}`}
                    >
                      📖 {s.matnName}
                    </span>
                  </div>
                  <button
                    ...
                    className={`opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-500/10 hover:text-red-650 transition-all ${
                      currentSessionId === s.id
                        ? "text-white hover:bg-bg-secondary/20"
                        : "text-text-muted"
                    }`}
                    title="حذف الأرشيف"
                  >
              ...
              {sessions.length === 0 && (
                <p className="text-[10px] text-text-muted font-bold text-center py-6">
      ...
      <div className="md:col-span-3 flex flex-col h-[550px] justify-between">
        {/* Chat header banner */}
        <div className="p-4 border-b border-border-primary flex items-center justify-between bg-bg-tertiary/50">
          <div className="flex items-center gap-2">
            <span className="p-2 bg-brand-light rounded-xl text-brand-primary">
              <Bot className="w-5 h-5 animate-pulse" />
            </span>
            <div className="text-right">
              <h3 className="font-black text-xs md:text-sm text-text-primary flex items-center gap-1">
                مساعد تجاويد الذكي
              </h3>
              <p className="text-[9px] text-text-secondary font-medium">
                اطرح أي مسألة تجويدية أو نحوية
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 py-2 bg-bg-tertiary/30 flex flex-wrap gap-1.5 border-b border-border-primary">
          <button
            onClick={() =>
              handleActionClick(
                `اقترح عليّ خطة لحفظ وضبط متن ${matnName} بأبوابه وقواعده`,
              )
            }
            className="px-2.5 py-1.5 bg-amber-500/10 text-white text-[10px] font-black rounded-lg border border-amber-500/20 hover:bg-amber-500/20 transition-all text-right cursor-pointer"
          >
            📋 جدول دراسة وحفظ
          </button>
          <button
            onClick={() =>
              handleActionClick(
                `لخص لي أهم الأحكام والقواعد التجويدية في متن ${matnName}`,
              )
            }
            className="px-2.5 py-1.5 bg-brand-light0/10 text-emerald-850 text-[10px] font-black rounded-lg border border-emerald-500/20 hover:bg-brand-light0/20 transition-all text-right cursor-pointer"
          >
          ...
          <button
            onClick={() =>
              handleActionClick(
                `ولد لي اختباراً سريعاً من 3 أسئلة في أحكام التجويد الأساسية لـ ${matnName}`,
              )
            }
            className="px-2.5 py-1.5 bg-blue-500/10 text-white text-[10px] font-black rounded-lg border border-blue-500/20 hover:bg-blue-500/20 transition-all text-right cursor-pointer"
          >
            ❓ مسابقة تجويد سريعة
          </button>
        </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto custom-scrollbar space-y-4 bg-bg-primary/50">
          {messages.map((m) => (
            <div
              key={m.id}
            ...
            >
              ...
              <div className="flex flex-col space-y-1.5 flex-1">
                <div
                  className={`p-3.5 rounded-2xl text-[11px] font-medium leading-relaxed text-right text-text-primary whitespace-pre-wrap ${
                    m.sender === "user"
                      ? "bg-bg-tertiary rounded-tr-none border border-border-primary shadow-sm"
                      : "bg-bg-tertiary rounded-tl-none border border-border-primary shadow-sm"
                  }`}
                >
                  {m.text}
                </div>
          ...
          {loading && (
            <div className="flex items-start gap-2.5 max-w-[85%] mr-auto">
              <div className="p-1.5 rounded-xl bg-brand-light text-emerald-950">
                <Loader2 className="w-4.5 h-4.5 animate-spin" />
              </div>
              <div className="p-3 bg-bg-tertiary text-right rounded-2xl rounded-tl-none border border-border-primary text-[10px] font-bold text-text-muted">
        ...
        <div className="p-3 border-t border-border-primary bg-bg-tertiary">
          ...
          >
            <input
              ...
              className="flex-1 px-4 py-2 rounded-xl text-xs bg-bg-secondary border border-border-primary outline-none focus:border-brand-primary text-right font-medium"
            />
            <button
              type="submit"
              disabled={loading || !inputMsg.trim()}
              className="px-4 py-2 bg-brand-primary text-white rounded-xl shadow-md hover:bg-emerald-900 transition-colors disabled:opacity-40 cursor-pointer"
            >
